   ...
   HttpServletResponse res){
    ...
    switch (path){
      ...
      try{
        ...
        List<Usuario> registro = uSer.list(nomeUsuario);
        
        Boolean out = false;
        if(registro.isEmpty()){
          System.out.println("<h1>Nada</h1>");
        }
        else{ 
          for(Usuario linha: registro){
            if(linha.getSenha().equals(senha)){
              out = true;
              break;
            }
            else{
              System.out.println("NAOOO FOOI");
            }

            System.out.println(linha.getLogin());
          }
